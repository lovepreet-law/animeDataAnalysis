...
ggplot(anime %>% filter(!is.na(score)), aes(y = scored_by, x = score)) +
  ...
  geom_smooth(formula = y ~ x, method = "loess", se = FALSE, aes(group = NULL), linewidth = .7)  # "loess" per una linea non lineare
...
# Creo violin plot
ggplot(anime_genres, aes(x = reorder(genres, score), y = score, fill = genres)) +
